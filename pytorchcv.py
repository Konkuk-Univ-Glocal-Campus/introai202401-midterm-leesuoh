...
# Python에서 FashionMNIST 데이터셋을 불러와서 처리하는 과정
...
def load_mnist(batch_size=64): # load_mnist라는 이름의 함수를 정의하고, 이 함수는 기본적으로 batch_size 매개변수를 64로 설정합니다. 이 매개변수는 데이터를 얼마나 많은 단위로 나눌지 결정
    builtins.data_train = torchvision.datasets.FashionMNIST('./temp',
    ...
    builtins.data_test = torchvision.datasets.FashionMNIST('./temp',  
...
def plot_results(hist): # plot_results라는 함수를 정의하는데 hist라는 이름의 딕셔너리를 매개변수로 받는데 학습과 검증 과정의 정확도와 손실이 배열 형태로 저장되어 있음
    ...
    plt.plot(hist['test_acc'], label='Test acc') # hist 딕셔너리에서 검증 정확도(test_acc)를 추출하여 그래프로 그리는데 라벨을 'Test acc'로 지정
    ...
    plt.plot(hist['test_loss'], label='Test loss') # hist 딕셔너리에서 검증 손실(test_loss)을 추출하여 그래프로 그리는데 라벨을 'Test loss'로 지정
